...
namespace GhostSpectator.Patches
{
    ...
    class DrawRandomTeamPatch
    {
        public static bool Prefix(RespawnTickets __instance, ref SpawnableTeamType __result)
        {
            ...
            {
                if ((allHub.Value.characterClassManager.CurClass == RoleType.Spectator && !allHub.Value.serverRoles.OverwatchEnabled) || API.IsGhost(Player.Get(allHub.Value)))
                {
                    flag = true;
                    break;
                }
            ...
            foreach (KeyValuePair<SpawnableTeamType, int> ticket in __instance._tickets)
            {
                ...
                else
                {
                    ...
                    if (ticket.Value == 0 && RespawnWaveGenerator.SpawnableTeams.TryGetValue(ticket.Key, out spawnableTeam) && spawnableTeam.LockUponZero)
